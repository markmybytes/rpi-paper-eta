...
class Controller(controller.Controller, controller.Partialable):
    ...
    def __init__(self, is_partial: bool) -> None:
        ...
        try:
            from .epd_lib import epd1in54_V2
        except ImportError:
            from epd_lib import epd1in54_V2
        self.epdlib = epd1in54_V2.EPD()

    def initialize(self):
        if type(self)._inited:
            return

        if ((self.is_partial and self.epdlib.init(True) == -1)
                or (not self.is_partial and self.epdlib.init(False) != -1)):
